...
let rarityWeights = [0, 1, 2, 3, 4, 5];

export default function simulate(items) {
    const input = arguments[0] instanceof Array ? items : arguments;
    const ingredients = new Array(attributeNames[lang].length);
    ingredients.fill(0);
    const material = [];
    let sum = 0;
    for (i = 0; i < input.length; ++i) {
        ...
        if (input[i] != undefined) {
            ...
            if (material[input[i]["type"]] == undefined)
                material[input[i]["type"]] = 1;
            else ++material[input[i]["type"]];
        }
    }
    const result = equips.slice(0);
    ...
    for (i = 0; i < result.length; ++i) {
        while (
            i < result.length &&
            (!result[i].alchemy || result[i].available.indexOf(lang) == -1)
            )
            result.splice(i, 1);
        if (i >= result.length) break;
        result[i]["chance"] = result[i]["weight"]; // 复制一份，避免修改原值
        for (j in result[i]["core"]) {
            if (result[i]["core"][j] > 0 && !(result[i]["core"][j] <= material[j])) {
    ...
    for (i = 0; i < result.length; ++i) {
        result[i]["pr"] =
            ((result[i]["chance"] / sum[result[i]["rarity"]]) *
                rarityWeights[result[i]["rarity"]]) /
            rws;
